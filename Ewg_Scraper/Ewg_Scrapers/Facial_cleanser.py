...
CATEGORY_NAME = "Facial_cleanser"
START_PAGE = 1
END_PAGE = 274
OUTPUT_FILE = Path(__file__).with_name("Facial_cleanser.csv")
...
def _configure() -> dict:
    ...
    cfg["RETRIES_PER_PRODUCT"] = max(1, cfg.get("RETRIES_PER_PRODUCT", 2))
    cfg["PRODUCT_HARD_TIMEOUT"] = 10
    cfg["PAGE_BATCH_SIZE"] = max(10, cfg.get("PAGE_BATCH_SIZE", 500))
    return cfg
...
def _ensure_urls(driver, cfg) -> List[str]:
    urls, names = load_url_cache(CATEGORY_NAME, START_PAGE, END_PAGE)
    ...
    if refresh:
        ...
        if cfg.get("USE_URL_CACHE"):
            save_url_cache(CATEGORY_NAME, START_PAGE, END_PAGE, urls, names)
